...
import { Cpu, ChevronUp, ChevronDown, Check, Sparkles } from 'lucide-react';
...
export const AIProviderSelector = () => {
  ...
  return (
    <div className="relative inline-block" ref={dropupRef}>
      ...
      {/* Menu Dropup (Expande para cima) */}
      {isOpen && (
        <div className="absolute bottom-full left-0 mb-2 w-72 max-h-80 overflow-y-auto bg-[#181C14] border border-[#3C3D37] rounded-2xl shadow-2xl p-2.5 z-50 backdrop-blur-xl animate-fadeIn space-y-4">
          ...
          {providerEntries.map(([pName, modelsDict]) => {
            ...
            return (
              <div key={pName} className="space-y-2">
                {/* Cabeçalho do Provedor com estilização de IDE (FONTE MONO/SUBTLE, MAIOR SEPARAÇÃO COM GAP) */}
